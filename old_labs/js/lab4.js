...
function mostrarTabla() {
  const n = Number(document.getElementById('numTabla').value);
...
function verificarSuma(a, b, start) {
  const respuesta = Number(document.getElementById('respSuma').value);
  const end = Date.now();
  const correcto = respuesta === (a+b);
  const ms = (end-start)/1000;
  const msg = correcto ? '¡Correcto!' : `Incorrecto. La respuesta era ${a+b}`;
...
function contador(arr) {
  let neg = 0; let ceros = 0; let pos = 0;
  for (const x of arr) {
    if (x < 0) neg++;
    else if (x === 0) ceros++;
    else pos++;
...
function mostrarContador() {
  const arr = document.getElementById('numsContador').value.split(',').map(Number);
...
function mostrarPromedios() {
  const matriz = document.getElementById('matrizProm').value.split(';').map((row)=>row.split(',').map(Number));
...
function mostrarInverso() {
  const num = Number(document.getElementById('numInverso').value);
